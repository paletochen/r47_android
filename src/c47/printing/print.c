...
#define RETURN_IF_PRINT_OFF            \
  do {                                 \
    if (!getSystemFlag(FLAG_PRTACT)) { \
      return;                          \
    }                                  \
  } while (0)
#define BREAK_IF_EXIT            \
  do {                           \
    if (key_pop() == KEY_EXIT) { \
      break;                     \
    }                            \
...
uint8_t charMap(uint16_t charCode) {
  uint8_t i;
  if (lastFunc ==
      ITM_PRINTERALPHA) {       // map the control character symbols only for
                                // pr_alpha, otherwise print the symbol
...
int16_t findPrinterGlyph(const printerFont_t* font, uint16_t charCode) {
  ...
  return font->numberOfGlyphs - 1;  // Last character of the font is for
                                    // charCodes not supported by the font
}
...
static bool_t _exitKeyPressed() {
  ...
  return currentKeyCode ==
         32;  // EXIT1 / EXIT key //Do not us gtk_events_pending() as it
              // triggers for timers too
#endif                   // PC_BUILD
}
...
void prepareNewLine(void) {
  ...
  while (printer_busy_for(PRINT_GRA_LN) &&
         i < 50) {            // return after 5.0s anyway
...
void printIR(uint8_t c) {  // prints a single character
  ...
  // Reset the CL_DROP timer to avoid missing double press on <- when tracing
  // the CLX instruction
...
void printGraphic8(uint8_t glen, const unsigned char* graphic) {
  ...
  if (glen > 0) {
    sendByteIR(27);  // Set bit image (82240 graphic character printing : ESC
                     // Xchar [Ci]
    sendByteIR(len);
    if (printerColumn != 0) {
...
void printGraphic24(uint8_t glen, const unsigned char* graphic) {
  if (glen > 0) {
    printerColumn += glen;
    sendByteIR(
        27);  // Set bit image (24 pin double density) : ESC NULL * 33 n1 n2 [d]
    sendByteIR(0);
    sendByteIR(42);
    sendByteIR(33);
    sendByteIR(glen / 3);
...
void printMartelGlyph(uint16_t charCode) {
  ...
  if (glyphId == 255) {
    printGlyph8(charCode,
                &printerFont8);  // Character not in the Martel high res font
  } else {
...
void printGlyph24(uint16_t charCode, const font_t* font) {
  ...
  for (row = glyph->rowsGlyph + glyph->rowsBelowGlyph;
       row > glyph->rowsBelowGlyph; row--) {
    ...
    for (col = 0; col < glyph->colsGlyph; col++) {
      ...
      if (byte & 0x80) {  // MSB set
        graphic[graphic_byte] =
            graphic[graphic_byte] |
            (1 << ((row_scaled - 1) % 8));  // Set graphic pixel
      } else {
        graphic[graphic_byte] =
            graphic[graphic_byte] &
            ~(1 << ((row_scaled - 1) % 8));  // Clear graphic pixel
      }
    ...
    switch (mode) {
      case PMODE_DEFAULT:  // Mixed character and graphic printing
        ...
        if (c & 0x80) {  // Unicode
          ...
          c = charMap(charCode);
          if (c == 0) {  // Not in the 82240 roman character set, need to print
                         // graphic
...
static void _realStringToPrint(char* realString, int16_t max_len) {
  ...
  k = 0;
  if (stringGlyphLength(realString) >
      max_len) {  // if string too long for printer
    for (i = 0; i < len; i++) {
      if ((realString[i] == 'e') || (realString[i] == 'E')) {
        expLen = len - i;
        from = &realString[i];
        pos = 1;
        for (j = 0; j < max_len - expLen - 1;
             j++) {  // Find the location to move the exponent to fit within
                     // max_len characters
        ...
        memmove(
            to, from,
            expLen + 1);  // Move exponent within the first max_len characters
        break;
...
static void _real34ToPrintString(real34_t* real34, uint16_t amMode,
                                 char* realString, int16_t maxWidth) {
  uint8_t grpGroupingLeftOld = grpGroupingLeft;
  uint8_t grpGroupingRightOld = grpGroupingRight;

  grpGroupingLeft = 0;
  grpGroupingRight = 0;
  real34ToDisplayString(real34, amMode, realString, &standardFont, maxWidth, 16,
                        true, false, true);
    ...
    switch ((uint8_t)realString[i]) {
      ...
      case 0xa1:
        if (realString[i + 1] == STD_SUP_MINUS[1]) {  // Exponent sign found
          ...
          j++;
          i++;
        } else if ((realString[i + 1] >= STD_SUP_0[1]) &&
                   (realString[i + 1] <= STD_SUP_9[1])) {
          realString[j] = realString[i + 1] -
                          0x30;  // replace superscript digits by normal digits
          j++;
          i++;
        } else {
          i++;
...
static void _complex34ToPrintString(real34_t* registReal34,
                                    real34_t* registImag34, uint16_t tagAngle,
                                    uint16_t tagPolar, char* realString) {
  int16_t max_len = 17;
  real_t real, imagIc;
  ...
  if (tagPolar) {  // polar mode
    ...
    int maxExponent =
        max(real.exponent + real.digits, imagIc.exponent + imagIc.digits);
    c.digits = (SHOWMODE ? 39
                         : max(0, maxExponent) +
                               NUMBER_OF_DISPLAY_REAL_CONTEXT_DIGITS +
                               2);  // add 2 guard digits for Taylor etc.
    realRectangularToPolar(&real, &imagIc, &real, &imagIc,
                           &c);  // imagIc in radian
    c.digits =
        (SHOWMODE
             ? 39
             : 3 + NUMBER_OF_DISPLAY_REAL_CONTEXT_DIGITS);  // converting from
                                                            // radians to grad
                                                            // is the worst,
                                                            // i.e. x 2E2 / pi,
                                                            // which requires 3
  ...
  _realStringToPrint(
      tmpString,
      max_len);  // Adjust the real part lenght (same as a standard real)
  ...
  _realStringToPrint(
      imaginaryPart,
      max_len + (tagPolar ? 1 : 0));  // Adjust the imaginary part length
}

//
//  Print a single register
//
void printReg(uint16_t regist, const char* label, bool_t eq, printArea_t where,
              bool prSigma) {
  uint16_t tagAngle;
  uint16_t tagPolar;
  real34_t* real34;
  ...
  // int16_t max_width = ((where == LINE_FULL) || (where == LINE_NOLF) ? 153 :
  // 99);
  if (prSigma) {
    max_len -= 1;
    // max_width -= 9;
  ...
  //  dtLongInteger     =  0,  ///< Z arbitrary precision integer DONE dtReal34
  //  =  1,  ///< R double precision real (128 bits)               DONE
  //  (fractions are printed as reals, not as fractions) dtComplex34       =  2,
  //  ///< C double precision complex (2x 128 bits)         DONE dtTime =  3,
  //  ///< Time                                             DONE dtDate =  4,
  //  ///< Date in various formats                          DONE dtString =  5,
  //  ///< Alphanumeric string                              Need to complete the
  //  printerFont8 dtReal34Matrix    =  6,  ///< Double precision vector or
  //  matrix                DONE dtComplex34Matrix =  7,  ///< Double precision
  //  complex vector or matrix        DONE dtShortInteger    =  8,  ///< Short
  //  integer (64 bit)                           DONE dtConfig          =  9,
  //  ///< Configuration                                    DONE, print "Config.
  //  data"
  ...
  switch (getRegisterDataType((calcRegister_t)regist)) {
    case dtString:
      ...
      if (where == LINE_FULL) {
        addChrBothSides(34, tmpString);  // Add quotes only for standard print
                                         // reg, not for print XY
      }
      break;

    case dtReal34:
      _real34ToPrintString(REGISTER_REAL34_DATA(regist),
                           getRegisterAngularMode(regist), tmpString,
                           max_len * 9);
      _realStringToPrint(tmpString,
                         max_len);  // Fit the number on a single line
      break;
    ...
    case dtReal34Matrix: {
      ...
      sprintf(tmpString, "[ %" PRIu16 "x%" PRIu16 " Matrix ]", rows,
              columns);  // Matrix header
      printJustified(tmpString);
      for (i = 1; i <= rows; i++) {
        for (j = 1; j <= columns; j++) {
          sprintf(tmpString, "%" PRIu16 ":%" PRIu16 "=", i,
                  j);  // Matrix element
    ...
    case dtComplex34Matrix: {
      ...
      sprintf(tmpString, "[ %" PRIu16 "x%" PRIu16 " Cpx Matrix ]", rows,
              cols);  // Matrix header
      printJustified(tmpString);
      for (i = 0; i < rows; i++) {
        for (j = 0; j < cols; j++) {
          sprintf(tmpString, "%" PRIu16 ":%" PRIu16 "=", i + 1,
                  j + 1);  // Matrix element
          printLine(tmpString, 0);
          real34 = VARIABLE_REAL34_DATA(&matrix->matrixElements[i * cols + j]);
          imag34 = VARIABLE_IMAG34_DATA(&matrix->matrixElements[i * cols + j]);
          _complex34ToPrintString(real34, imag34, tagAngle, tagPolar,
                                  tmpString);
          printJustified(tmpString);
        }
      }
  ...
  if (label == NULL &&
      (where == LINE_FULL || (where == LINE_NOLF))) {  // Padding for PRX
    uint16_t i, padding;
    ...
    if (glen > 17) {
      ...
      for (i = 0; i < padding; i++) {
        strcat(tmpString,
               " ");  // pad string to ensure "***" will be right aligned
      }
    }

    if (where == LINE_FULL) {
      strcpy(tmpString + strlen(tmpString),
             "    ***");  // End line with "    ***" as on the HP-41 and 42
    }
  }
...
void cmdPrint(uint16_t arg, printArgument_t op) {
  ...
  if (!getSystemFlag(FLAG_PRTACT)) {
    if (getSystemFlag(FLAG_PRTEN) || ((programRunStop != PGM_RUNNING) &&
    ...
    for (y = SCREEN_HEIGHT - 1; y >= 0; y--) {
      ...
      for (i = 2; i >= 0; i--) {
        ...
        if ((offset > 0) && (offset < 50)) {
          data = ((((~*(line_addr + offset)) << 4) & 0xf0) |
                  (((~*(line_addr + offset - 1)) >> 4) & 0x0f));
        } else if (offset == 0) {
          data = (((~*(line_addr + offset)) << 4) & 0xf0) |
                 0x08;  // Print with right vertical line
        } else {
          data = 0x10 | (((~*(line_addr + offset - 1)) >> 4) &
                         0x0f);  // Print with left vertical line
        }
        sendByteIR(data);
      }
    }
...
static bool_t _printRegRange(uint16_t firstRegisterNo,
                             uint16_t lastRegisterNo) {
  uint16_t regist;
  currentKeyCode = 255;
...
void printTraceErrorFunction(int16_t func, char* errorString) {
  if ((calcMode != CM_NORMAL) ||
      ((tam.mode) && ((func == ITM_BACKSPACE) || (func == ITM_EXIT1)))) {
    return;  // do Trace only in normal mode
  }
  if (getSystemFlag(FLAG_TRACE) &&
      getSystemFlag(FLAG_PRTACT)) {  // Trace mode and printer active
...
void printTraceX(uint16_t where) {
  if ((getSystemFlag(FLAG_TRACE) || getSystemFlag(FLAG_NORM)) &&
      getSystemFlag(FLAG_PRTACT)) {  // Trace or Norm mode and printer active
    if (getSystemFlag(FLAG_TRACE) && getSystemFlag(FLAG_NORM) &&
        (where == LINE_FULL)) {
      _printRegRange(getSystemFlag(FLAG_SSIZE8) ? REGISTER_D : REGISTER_T,
                     REGISTER_X);  // Trace with stack
      print_lf();
    } else {
      printReg(REGISTER_X, NULL, false, where,
               false);  // Print register X without name header
    }
...
void printTraceMatElement(uint16_t where) {
  if (getSystemFlag(FLAG_TRACE) &&
    ...
    if (getRegisterDataType(matrixIndex) == dtReal34Matrix) {
      ...
      real34Copy(&matrix->matrixElements[i * matrix->header.matrixColumns + j],
                 REGISTER_REAL34_DATA(TEMP_REGISTER_1));
    } else {
      ...
      complex34Copy(
          &matrix->matrixElements[i * matrix->header.matrixColumns + j],
          REGISTER_COMPLEX34_DATA(TEMP_REGISTER_1));
    }
    printReg(TEMP_REGISTER_1, NULL, false, where,
             false);  // Print temporary register 1 without name header
...
void printTraceString(char* string, uint16_t where) {
  ...
  if ((getSystemFlag(FLAG_TRACE) || getSystemFlag(FLAG_NORM)) &&
      getSystemFlag(FLAG_PRTACT)) {  // Trace or Norm mode and printer active
    reallocateRegister(TEMP_REGISTER_1, dtString, TO_BLOCKS(lenInBytes),
                       amNone);
...
void printTraceTI() {
  ...
  if (getSystemFlag(FLAG_TRACE) &&
    ...
    if ((programRunStop != PGM_RUNNING) &&
        (programRunStop !=
         PGM_SINGLE_STEP)) {  // Not executing a program instruction
      tmpString[0] = 0;
...
void _getRegisterLabel(uint16_t registerNo, char* label) {
  ...
  } else if (FIRST_LOCAL_REGISTER <= registerNo &&
             registerNo <= LAST_LOCAL_REGISTER) {
    sprintf(label, "R.%03d", registerNo - 100);
  } else if (FIRST_NAMED_VARIABLE <= registerNo &&
             registerNo <= LAST_NAMED_VARIABLE) {
    sprintf(label, "%s",
            (char*)allNamedVariables[registerNo - FIRST_NAMED_VARIABLE]
                    .variableName +
                1);
  } else if (FIRST_NAMED_RESERVED_VARIABLE <= registerNo &&
             registerNo <= LAST_RESERVED_VARIABLE) {
    sprintf(label, "%s",
            (char*)allReservedVariables[registerNo - FIRST_RESERVED_VARIABLE]
...
void printInputPrompt(uint16_t func, uint16_t regist) {
  if ((getSystemFlag(FLAG_TRACE) || getSystemFlag(FLAG_NORM)) &&
      getSystemFlag(FLAG_PRTACT)) {  // Trace or Norm mode and printer active
    if (getSystemFlag(FLAG_PRTEN) ||
        ((programRunStop != PGM_RUNNING) &&
         (programRunStop != PGM_SINGLE_STEP) &&
         (programRunStop !=
          PGM_WAITING))) {  // No printing in a program if PRTEN cleared
      if (func == ITM_PROMPT) {
        printReg(regist, NULL, false, LINE_ASIS,
                 false);  // Print register left justified without name header
        print_lf();
#if defined(PC_BUILD) && defined(MONITOR_IRPRINT)
        printf("**[DL]** Trace: %s\n", tmpString);
        fflush(stdout);
        ...
        label[len++] = '?';
        label[len] = 0;
        printReg(regist, label, false, LINE_ASIS,
                 false);  // Print register left justified with name header
        print_lf();
#if defined(PC_BUILD) && defined(MONITOR_IRPRINT)
        printf("**[DL]** Trace: %s\n", tmpString);
        fflush(stdout);
...
void printViewAview(uint16_t func, uint16_t regist) {
  if (getSystemFlag(
          FLAG_PRTACT)) {  // Man, Trace or Norm mode and printer active
    if (getSystemFlag(FLAG_PRTEN) ||
        ((programRunStop != PGM_RUNNING) &&
         (programRunStop !=
          PGM_SINGLE_STEP))) {  // No printing in a program if PRTEN cleared
      if (func == ITM_VIEW) {
        char label[16];
        _getRegisterLabel(regist, label);
        printReg(regist, label, true, LINE_ASIS,
                 false);  // Print register left justified with name header
        print_lf();
#if defined(PC_BUILD) && defined(MONITOR_IRPRINT)
        printf("**[DL]** Trace: %s=%s\n", label, tmpString);
    ...
    if (getSystemFlag(FLAG_PRTEN) &&
        (programRunStop ==
         PGM_RUNNING)) {  // Stop in a program if PRTEN and not PRTACT
...
void printTrace(int16_t func, uint16_t param) {
  ...
  if ((getSystemFlag(FLAG_TRACE) || getSystemFlag(FLAG_NORM)) &&
    ...
    if ((programRunStop != PGM_RUNNING) &&
      ...
      if (func < 0) {         // Menu
        if (func == -MNU_DYNAMIC) {
          ...
          printJustified(
              indexOfItems[-func].itemSoftmenuName);  // Predefined Menu
#if defined(PC_BUILD) && defined(MONITOR_IRPRINT)
          printf("**[DL]** Trace: %s\n", indexOfItems[-func].itemSoftmenuName);
          fflush(stdout);
#endif  // PC_BUILD
        }
      } else if ((func != ITM_PRINTERADV) &&
                 (func != ITM_PRINTERX)) {  // Function (don't trace printer ADV
                                            // nor PRX)
        char buffer[16];
        xcopy(buffer, tmpString,
              16);  // Save tmpString content for dynamic menus
        nameAlias(func, tmpString);
        if ((indexOfItems[func].func != addItemToBuffer) &&
            (indexOfItems[func].param != NOPARAM) &&
          ...
          if ((tam.mode == TM_VALUE) &
              !tam.indirect) {  // Needs to be the first if to correctly catch
                                // values
            ...
            if (param == MNU_DYNAMIC) {
              sprintf(traceBuffer,
                      " " STD_LEFT_SINGLE_QUOTE "%s" STD_RIGHT_SINGLE_QUOTE,
                      buffer);
            } else {
              sprintf(traceBuffer,
                      " " STD_LEFT_SINGLE_QUOTE "%s" STD_RIGHT_SINGLE_QUOTE,
                      indexOfItems[tam.value].itemCatalogName);
            }
          ...
          } else if ((param >= FIRST_NAMED_VARIABLE) &&
                     (param <= LAST_NAMED_VARIABLE)) {
            if (!tam.indirect) {
              strcat(tmpString, " ");
            }
            strcat(tmpString, STD_LEFT_SINGLE_QUOTE);
            strcat(tmpString,
                   (char*)allNamedVariables[param - FIRST_NAMED_VARIABLE]
                           .variableName +
                       1);
            strcat(tmpString, STD_RIGHT_SINGLE_QUOTE);
          } else if ((param >= FIRST_NAMED_RESERVED_VARIABLE) &&
            ...
            strcat(tmpString, STD_LEFT_SINGLE_QUOTE);
            strcat(tmpString,
                   (char*)allReservedVariables[param - FIRST_RESERVED_VARIABLE]
                           .reservedVariableName +
                       1);
            strcat(tmpString, STD_RIGHT_SINGLE_QUOTE);
          } else if ((tam.mode == TM_LABEL || tam.mode == TM_LBLONLY ||
                      func == ITM_XEQ) &&
                     !tam.indirect) {
            if (param < 99) {  // Local label from 00 to 99
              sprintf(traceBuffer, " %02u", param);
            } else if (param <=
                       LAST_UC_LOCAL_LABEL) {  // Local label from A to L
            ...
            } else if ((param >= FIRST_LABEL) &&
                       (param <= LAST_LABEL)) {  // Alpha labels
              strcat(tmpString, " " STD_LEFT_SINGLE_QUOTE);
              uint16_t strLength = stringByteLength(tmpString);
              uint8_t lblNameLen = boundProgramNameLength(
                  labelList[param - FIRST_LABEL].labelPointer + 1,
                  *(labelList[param - FIRST_LABEL].labelPointer));
              xcopy(tmpString + strLength,
                    labelList[param - FIRST_LABEL].labelPointer + 1,
                    lblNameLen);
            ...
            if (param < FLAG_X) {  // Global flag from 00 to 99
              sprintf(traceBuffer, " %02u", param);
            } else if (FLAG_X <= param &&
                       param <= FLAG_K) {  // Lettered flag from X to K
              sprintf(traceBuffer, " %c", registerFlagLetters[param - FLAG_X]);
            } else if (param <=
                       LAST_LOCAL_FLAG) {  // Local flag from .00 to .31
              sprintf(traceBuffer, " .%02d", param - FIRST_LOCAL_FLAG);
            } else if (param <
                       FLAG_M) {  // Local flag from .32 to .98 are illegal
              sprintf(traceBuffer, " %02u", param);
            } else if (param <=
                       FLAG_W) {  // Lettered flag from M to S and E to W
              sprintf(
                  traceBuffer, " %c",
                  registerFlagLetters[param - FLAG_M + (FLAG_K - FLAG_X + 1)]);
            } else if (param & 0x8000) {  // System flag
              param &= 0x3fff;
              if (param < 64) {
                sprintf(traceBuffer,
                        " " STD_LEFT_SINGLE_QUOTE "%s" STD_RIGHT_SINGLE_QUOTE,
                        indexOfItems[param + SFL_TDM24].itemSoftmenuName);
              } else {
                sprintf(traceBuffer,
                        " " STD_LEFT_SINGLE_QUOTE "%s" STD_RIGHT_SINGLE_QUOTE,
                        indexOfItems[param + SFL_MONIT - 64].itemSoftmenuName);
              }
            }
          } else if ((tam.mode == TM_CMP) && (param == TEMP_REGISTER_1) &&
                     !tam.indirect) {
            sprintf(traceBuffer,
                    real34IsZero(REGISTER_REAL34_DATA(TEMP_REGISTER_1))
                        ? " 0."
                        : " 1.");
          } else {
            if (param < 99) {  // Global register from 00 to 99
              ...
              sprintf(traceBuffer, "%s",
                      indexOfItems[ITM_REG_X + param - REGISTER_X]
                          .itemSoftmenuName);
            } else if (param <= REGISTER_W) {  // Lettered register from M to S
                                               // and E to W
              if (!tam.indirect) {
                strcat(tmpString, " ");
              }
              sprintf(traceBuffer, "%s",
                      indexOfItems[ITM_REG_M + param - REGISTER_M]
                          .itemSoftmenuName);
            } else if ((param >= FIRST_LOCAL_REGISTER) &&
                       (param <= LAST_LOCAL_REGISTER)) {  // Local register from
                                                          // .00 to .98
              if (!tam.indirect) {
                strcat(tmpString, " ");
              }
    ...
    } else if (getSystemFlag(FLAG_TRACE)) {  // Program running or single
                                             // stepping - Trace mode only
      ...
      if (func == ITM_LBL) {
        ...
        fflush(stdout);
                 ...
                 0) {  // Don't trace menu names during program execution
  ...
  for (line = firstLine; line <= lastLine; line++) {
    ...
    if (getSystemFlag(FLAG_TRACE)) {  // Compact program format
      if (isLabel) {
        ...
        sprintf(
            tmpString, " %02d",
            firstDisplayedLocalStepNumber + line - lineOffset + lineOffsetTam);
...
static uint16_t _getUnicodeValue(calcRegister_t regist) {
  ...
  if (getRegisterDataType(regist) == dtReal34) {
    ...
    if (longIntegerCompareUInt(lgInt, 0) < 0 ||
        longIntegerCompareUInt(lgInt, 0x8000) >= 0) {
      displayCalcErrorMessage(ERROR_OUT_OF_RANGE, ERR_REGISTER_LINE,
                              REGISTER_X);
#if defined(PC_BUILD)
    ...
    convertShortIntegerRegisterToLongInteger(regist,
                                             lgInt);  // initialises lgInt
    longIntegerToUInt32(lgInt, value);
    longIntegerFree(lgInt);
  }
...
void fnP_User(uint16_t unusedButMandatoryParameter) {
#if defined(OPTION_IR_PRINTING)
  char label[256];  // a global label name is a 1-byte-length string, so up to
                    // 255 bytes
  currentKeyCode = 255;
  ...
  for (int i = 0; i < numberOfNamedVariables; i++) {
    xcopy(label, allNamedVariables[i].variableName + 1,
          allNamedVariables[i].variableName[0]);
    label[allNamedVariables[i].variableName[0]] = 0;
    if ((compareString(label, "STATS", CMP_NAME) == 0) ||
        (compareString(label, "HISTO", CMP_NAME) == 0) ||
        (compareString(label, "Mat_A", CMP_NAME) == 0) ||
        (compareString(label, "Mat_B", CMP_NAME) == 0) ||
        (compareString(label, "Mat_X", CMP_NAME) ==
         0)) {  // Don't include "STATS", "HISTO", "Mat_A", "Mat_B" and "Mat_X"
      continue;
    } else {
    ...
    if (checkOpCodeOfStep(step, ITM_LBL)) {  // LBL
      if (*(step + 1) > LAST_LOCAL_LABEL) {  // Global label
        ...
        if (firstProgramLabel) {
          sprintf(tmpString, "Prgm #%" PRIu16 "/%" PRIu16 "", programNumber,
                  numberOfPrograms);
          printJustified(tmpString);
...
#if defined(OPTION_IR_PRINTING)
#if defined(DMCP_BUILD)
    if (printerState.printer_model ==
        PRINTER_MARTEL) {  // Only for Martel on HW, not on simulator and not
                           // for 82240
      setPrinterSBI(true);
      resetShiftState();  // JM To avoid f or g top left of the screen, clear to
                          // make sure
      refreshScreen(80);
      printLcd();
...
void fnP_Alpha(uint16_t registerNo) {
  if (getSystemFlag(FLAG_PRTACT)) {  // Print to the printer)
    ...
    xcopy(
        tmpString, aimBuffer,
        ERROR_MESSAGE_LENGTH + AIM_BUFFER_LENGTH +
            NIM_BUFFER_LENGTH);  // backup portion of the "message buffer" area
    ...
    xcopy(aimBuffer, tmpString,
          ERROR_MESSAGE_LENGTH + AIM_BUFFER_LENGTH +
              NIM_BUFFER_LENGTH);  //   This total area must be less than the
...
TO_QSPI const summationRegisterName_t
    summationRegisterName[NUMBER_OF_STATISTICAL_SUMS] = {
        {"n"},
        {STD_SIGMA "X"},
        {STD_SIGMA "Y"},
  ...
  if (statisticalSumsPointer != NULL) {
    if (getSystemFlag(FLAG_PRTACT)) {  // Print to the printer
      ...
      for (regist = 0; regist < NUMBER_OF_STATISTICAL_SUMS; regist++) {
        convertRealToResultRegister(statisticalSumsPointer + regist,
                                    TEMP_REGISTER_1, amNone);
        printReg(TEMP_REGISTER_1, summationRegisterName[regist].name, true,
                 LINE_FULL, true);
        if (_exitKeyPressed()) {
          return;
        }
    ...
    moreInfoOnError("In function fnP_Sigma:",
                    "There is no statistical data available!", NULL, NULL);
  ...
  if (getSystemFlag(FLAG_PRTACT)) {  // Print to the printer
    ...
    switch (option) {
      case PRN_ALL:
        ...
        if (currentNumberOfLocalRegisters > 0) {
          exited = _printRegRange(FIRST_LOCAL_REGISTER,
        ...
        if (numberOfNamedVariables > 0) {
          exited =
              _printRegRange(FIRST_NAMED_VARIABLE, FIRST_NAMED_VARIABLE +
      ...
      case PRN_Xr:
        printReg(REGISTER_X, NULL, false, LINE_FULL,
                 false);  // Print register X without name header
        break;
        ...
        switch (getRegisterDataType(REGISTER_X)) {
          ...
          case dtTime: {
            switch (getRegisterDataType(REGISTER_Y)) {
              ...
              case dtTime: {
                printReg(
                    REGISTER_X, NULL, false, LINE_LEFT,
                    false);  // Print register X on the left half of the line
                printReg(
                    REGISTER_Y, NULL, false, LINE_RIGHT,
                    false);  // Print register Y on the right half of the line
                return;
              }
              default: {
                displayCalcErrorMessage(ERROR_INVALID_DATA_TYPE_FOR_OP,
                                        ERR_REGISTER_LINE, REGISTER_Y);
#if (EXTRA_INFO_ON_CALC_ERROR == 1)
                sprintf(errorMessage, "invalid data type %s for register Y",
                        getRegisterDataTypeName(REGISTER_Y, true, false));
                moreInfoOnError("In function fnP_All_Regs(PRN_XYr):",
          ...
          case dtReal34Matrix: {
            ...
            if (x.header.matrixColumns == 2) {
              for (int i = 0; i < x.header.matrixRows; ++i) {
                reallocateRegister(TEMP_REGISTER_1, dtReal34,
                                   REAL34_SIZE_IN_BYTES, amNone);
                real34Copy(&x.matrixElements[i * 2],
                           REGISTER_REAL34_DATA(TEMP_REGISTER_1));
                printReg(
                    TEMP_REGISTER_1, NULL, false, LINE_LEFT,
                    false);  // Print row i col 1 on the left half of the line
                real34Copy(&x.matrixElements[i * 2 + 1],
                           REGISTER_REAL34_DATA(TEMP_REGISTER_1));
                printReg(
                    TEMP_REGISTER_1, NULL, false, LINE_RIGHT,
                    false);  // Print row i col 2 on the right half of the line
              }
            } else if (x.header.matrixRows == 2) {
              for (int i = 0; i < x.header.matrixColumns; ++i) {
                reallocateRegister(TEMP_REGISTER_1, dtReal34,
                                   REAL34_SIZE_IN_BYTES, amNone);
                real34Copy(&x.matrixElements[i],
                           REGISTER_REAL34_DATA(TEMP_REGISTER_1));
                printReg(
                    TEMP_REGISTER_1, NULL, false, LINE_LEFT,
                    false);  // Print row 1 col i on the left half of the line
                real34Copy(&x.matrixElements[i + x.header.matrixColumns],
                           REGISTER_REAL34_DATA(TEMP_REGISTER_1));
                printReg(
                    TEMP_REGISTER_1, NULL, false, LINE_RIGHT,
                    false);  // Print row 2 col i on the right half of the line
              }
            } else {
              ...
              sprintf(errorMessage,
                      "cannot print xy when matrix size is %d x %d",
          ...
          case dtComplex34Matrix: {
            ...
            if (xc.header.matrixColumns == 2) {
              for (int i = 0; i < xc.header.matrixRows; ++i) {
                reallocateRegister(TEMP_REGISTER_1, dtComplex34, 0, amNone);
                complex34Copy(&xc.matrixElements[i * 2],
                              REGISTER_COMPLEX34_DATA(TEMP_REGISTER_1));
                printReg(
                    TEMP_REGISTER_1, NULL, false, LINE_LEFT,
                    false);  // Print row i col 1 on the left half of the line
                complex34Copy(&xc.matrixElements[i * 2 + 1],
                              REGISTER_COMPLEX34_DATA(TEMP_REGISTER_1));
                printReg(
                    TEMP_REGISTER_1, NULL, false, LINE_RIGHT,
                    false);  // Print row i col 2 on the right half of the line
              ...
              for (int i = 0; i < xc.header.matrixColumns; ++i) {
                reallocateRegister(TEMP_REGISTER_1, dtComplex34, 0, amNone);
                complex34Copy(&xc.matrixElements[i],
                              REGISTER_COMPLEX34_DATA(TEMP_REGISTER_1));
                printReg(
                    TEMP_REGISTER_1, NULL, false, LINE_LEFT,
                    false);  // Print row 1 col i on the left half of the line
                complex34Copy(&xc.matrixElements[i + xc.header.matrixColumns],
                              REGISTER_COMPLEX34_DATA(TEMP_REGISTER_1));
                printReg(
                    TEMP_REGISTER_1, NULL, false, LINE_RIGHT,
                    false);  // Print row 2 col i on the right half of the line
              ...
              sprintf(errorMessage,
                      "cannot print xy when matrix size is %d x %d",
                      xc.header.matrixRows, xc.header.matrixColumns);
              moreInfoOnError("In function fnP_All_Regs(PRN_XYr):",
                              errorMessage, NULL, NULL);
#endif  // (EXTRA_INFO_ON_CALC_ERROR == 1)
            }
...
#if (EXTRA_INFO_ON_CALC_ERROR == 1)
            sprintf(errorMessage, "invalid data type %s for register X",
                    getRegisterDataTypeName(REGISTER_X, true, false));
            moreInfoOnError("In function fnP_All_Regs(PRN_XYr):", errorMessage,
                            NULL, NULL);
#endif  // (EXTRA_INFO_ON_CALC_ERROR == 1)
            return;
          }
    ...
    switch (option) {
      case PRN_ALL:
        ...
        if (currentNumberOfLocalRegisters > 0) {
          stackregister_csv_out(
              FIRST_LOCAL_REGISTER,
              FIRST_LOCAL_REGISTER + currentNumberOfLocalRegisters - 1,
              !ONELINE);
        }
        if (numberOfNamedVariables > 0) {
          stackregister_csv_out(
              FIRST_NAMED_VARIABLE,
              FIRST_NAMED_VARIABLE + numberOfNamedVariables - 1, !ONELINE);
        }
        break;
