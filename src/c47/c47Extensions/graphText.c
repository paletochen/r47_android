...
/*-DMCP-*/ typedef struct {
  ...
  /*-DMCP-*/ int idx, nchr;  // Write index of buf[] (-1:error), number of
...
/*-DMCP-*/
/*-DMCP-*/
/*-DMCP-*/ int f_putc(TCHAR c /* A character to be output */,
                      FIL* fp /* Pointer to the file object */) {
  /*-DMCP-*/ putbuff pb;
  /*-DMCP-*/
...
/*-DMCP-*/ /*-----------------------------------------------------------------------*/
/*-DMCP-*/ int f_puts(const TCHAR* str /* Pointer to the string to be output */,
                      FIL* fp /* Pointer to the file object */) {
  /*-DMCP-*/ putbuff pb;
  /*-DMCP-*/
  /*-DMCP-*/ putc_init(&pb, fp);
...
/*-DMCP-*/
/*-DMCP-*/
/*-DMCP-*/ int16_t export_append_string_to_file(
    const char line1[TMP_STR_LENGTH], uint8_t mode, const char filedir[40]) {
...
/*-DMCP-*/ int16_t export_append_string_to_file_n(const char* line1,
                                                  uint8_t mode,
                                                  const char filedir[40]) {
  /*-DMCP-*/ char line[200];  // Line buffer
  /*-DMCP-*/ int fr;          // FatFs return code
                              /*-DMCP-*/
  /*-DMCP-*/ switch (mode) {
...
/*-DMCP-*/ }
/*-DMCP-*/
/*-DMCP-*/ int16_t save_text(const char* line1, uint8_t mode1, uint8_t mode2,
                             uint8_t mode3, int16_t nn, const char* dirfile) {
  /*-DMCP-*/ char rr[2];
  /*-DMCP-*/ rr[1] = 0;
  /*-DMCP-*/ switch (mode1) {
    /*-DMCP-*/ case OPEN:
  ...
  /*-DMCP-*/ switch (mode2) {
    /*-DMCP-*/ case WRITE:
      ...
      /*-DMCP-*/ while (ii != 0) {
        ...
        /*-DMCP-*/ if (export_append_string_to_file_n(rr, mode2, dirfile) !=
                       0) {
          /*-DMCP-*/ return 1;
        /*-DMCP-*/ }
        /*-DMCP-*/ ii--;
...
/*-DMCP-*/
/*-DMCP-*/
/*-DMCP-*/ int16_t export_string_to_filename(const char line1[TMP_STR_LENGTH],
                                             uint8_t mode, const char* dirname,
                                             const char* filename) {
...
/*-DMCP-*/
/*-DMCP-*/
/*-DMCP-*/ int16_t import_string_from_filename(char* line1, char* dirname,
                                               char* filename_short,
                                               char* filename, char* fallback,
                                               bool_t scanning) {
#if (VERBOSE_LEVEL >= 2)
...
/*-DMCP-*/ \
  ...
  /*-DMCP-*/ if (fr != FR_OK) {
...
#if (VERBOSE_LEVEL >= 2)
    /*-DMCP-*/ if (fr == 4) {
      /*-DMCP-*/ sprintf(line, "%s%d ", IOMsgs[6].itemName,
                         fr);  // Not found ID006 -->
    ...
    /*-DMCP-*/ }
    /*-DMCP-*/ else if (fr == 5) {
      /*-DMCP-*/ sprintf(line, "%s%d ", IOMsgs[7].itemName,
                         fr);  // No path ID007 -->
      /*-DMCP-*/ print_inlinestr(line, false);
      /*-DMCP-*/ sprintf(line, "File: %s \n", dirfile);
      /*-DMCP-*/ print_inlinestr(line, false);
    /*-DMCP-*/ }
    /*-DMCP-*/ else {
      /*-DMCP-*/ sprintf(line, "%s%d ", IOMsgs[2].itemName,
                         fr);  // File open error ID002-->
      /*-DMCP-*/ print_inlinestr(line, false);
      /*-DMCP-*/ sprintf(line, "File: %s \n", dirfile);
      /*-DMCP-*/ print_inlinestr(line, false);
...
#endif  // VERBOSE_LEVEL >= 1
#if (VERBOSE_LEVEL >= 1)
        /*-DMCP-*/ print_inlinestr((char*)IOMsgs[8].itemName,
                                   true);  //. Using fallback.
#endif                                     // VERBOSE_LEVEL >= 1
        /*-DMCP-*/ f_close(&fil);
  ...
  /*-DMCP-*/ line1[0] = 0;
  /*-DMCP-*/ f_getsline(
      line1, (scanning ? min(100, TMP_STR_LENGTH) : TMP_STR_LENGTH), &fil);
  /*-DMCP-*/ f_close(&fil);
  /*-DMCP-*/
#if (VERBOSE_LEVEL >= 1)
  ...
  /*-DMCP-*/ if (stringByteLength(line1) >= TMP_STR_LENGTH - 1) {
    /*-DMCP-*/ strcpy(line1, fallback);
#if (VERBOSE_LEVEL >= 1)
    /*-DMCP-*/ print_inlinestr((char*)IOMsgs[9].itemName,
                               true);  // ERROR too long file using fallback
#endif
    /*-DMCP-*/ return 1;
  /*-DMCP-*/ }
  /*-DMCP-*/
...
/*-DMCP-*/ void create_filename(char* fn) {  // fn must be in format ".STAT.TSV"
  ...
  /*-DMCP-*/ if (cancelFilename || (mem__32 == 0) ||
                 (tmp__32 > mem__32 + 120000) ||
                 (stringByteLength(filename_csv) > 10 &&
                  compareString(
                      filename_csv + (stringByteLength(filename_csv) - 9), fn,
                      CMP_NAME) != 0)) {
    /*-DMCP-*/  // Create file name
...
/*-DMCP-*/ int16_t export_xy_to_file(float x, float y) {
  /*-DMCP-*/ char* line =
      malloc(TMP_STR_LENGTH);  // Line buffer, on the heap: this calls
                               // export_append_string_to_file, and the pair
  ...
  /*-DMCP-*/ if (export_append_string_to_file(line, APPEND, filename_csv) !=
                 0) {
    /*-DMCP-*/  // ERROR ALREADY ANNOUNCED
    /*-DMCP-*/ free(line);
    /*-DMCP-*/ return 1;
...
int16_t export_string_to_filename(const char line1[TMP_STR_LENGTH],
                                  uint8_t mode, const char* dirname,
                                  const char* filename) {
  FILE* outfile;
  char dirfile[40];
...
int16_t import_string_from_filename(char* line1, char* dirname,
                                    char* filename_short, char* filename,
                                    char* fallback, bool_t scanning) {
#if (VERBOSE_LEVEL >= 2)
...
void create_filename(char* fn) {  // fn must be in format ".STAT.TSV"
  if (stringByteLength(filename_csv) > 9 &&
      compareString(filename_csv + (stringByteLength(filename_csv) - 8),
                    ".T47.TSV", CMP_NAME) == 0) {
  ...
  if (cancelFilename || (mem__32 == 0) || (tmp__32 > mem__32 + 120000) ||
      (stringByteLength(filename_csv) > 10 &&
       compareString(filename_csv + (stringByteLength(filename_csv) - 9), fn,
                     CMP_NAME) != 0)) {
    // Create file name
    time(&rawTime);
...
void printStatus(uint8_t row, const char* line1, uint8_t forced) {
  ...
  // g_line_x = showStringEnhanced(line1, &standardFont, g_line_x, g_line_y,
  // vmNormal, false, false, NO_compress, NO_raise, DO_Show, NO_LF);
  g_line_x = showString(line1, &standardFont, (uint32_t)g_line_x,
                        (uint32_t)g_line_y, vmNormal, true, true);
...
void print_linestr(const char* line1,
                   bool_t line_init) {  // prints one line at a time, filled
                                        // with dots on remaining line
  char l1[200];
  l1[0] = 0;
  int16_t ix = 0;
  int16_t ixx;
  ...
  while (ix < ixx && ix < 98 &&
         stringWidth(l1, &standardFont, true, true) <
             SCREEN_WIDTH - 12 - g_line_x) {
    xcopy(l1, line1, ix + 1);
    l1[ix + 1 + 1] = 0;
    ix = stringNextGlyph(line1, ix);
  }
  while (stringByteLength(l1) < 200 &&
         stringWidth(l1, &standardFont, true, true) <
             SCREEN_WIDTH - 12 - g_line_x) {
    strcat(l1, ".");
  }
  ...
  if (g_line_y < SCREEN_HEIGHT) {
    ixx = showString(l1, &standardFont, (uint32_t)g_line_x, (uint32_t)g_line_y,
                     vmNormal, true, true);
  }
  if (!line_init) {
    g_line_y += 20;
...
void print_numberstr(
    const char* line1,
    bool_t line_init) {  // ONLY N=ASCII NUMBERS AND E AND . //FIXED FONT
  ...
  if (g_line_y < SCREEN_HEIGHT) {
    ...
    while (line1[cnt] != 0 && g_line_x < SCREEN_WIDTH - 8 + 1) {
      tt[0] = line1[cnt];
      tt[1] = 0;
      g_line_x = showString(tt, &standardFont, (uint32_t)cnt * 8,
                            (uint32_t)g_line_y, vmNormal, true, true);
      cnt++;
    }
  }
...
void print_inlinestr(
    const char* line1,
    bool_t endline) {  // prints with or without newline at the end of the line
  ...
  ixx = stringByteLength(line1);
  while (ix < ixx && ix < 98 &&
         t_line_x + stringWidth(l1, &standardFont, true, true) <
             SCREEN_WIDTH - 12) {
    xcopy(l1, line1, ix + 1);
    l1[ix + 1] = 0;
    ix = stringNextGlyph(line1, ix);
  ...
  if (t_line_y < SCREEN_HEIGHT) {
    t_line_x =
        showString(l1, &standardFont, t_line_x, t_line_y, vmNormal, true, true);
  }
  if (endline) {
    t_line_y += 20;
    t_line_x = 0;
...
void print_Register_line(calcRegister_t regist, char* before, char* after,
                         bool_t line_init) {
  char* str = malloc(TMP_STR_LENGTH);  // off the stack: this calls
                                       // copyRegisterToClipboardString, which
                                       // holds another CLIPSTR buffer
  if (str == NULL) {
    return;
  }
